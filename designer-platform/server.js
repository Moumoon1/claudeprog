...
function checkPrerequisites() {
  ...
  try {
    ...
    try {
      ...
      missing.push('Pillow（安装: pip3 install Pillow）');
    } catch {
      missing.push('Python3（安装: brew install python3）');
    }
  }
  ...
  if (missing.length > 0) {
    console.log('\n' + '!'.repeat(50));
    console.log('  缺少必要依赖，以下功能可能不可用：');
    missing.forEach((m, i) => console.log(`  ${i + 1}. ${m}`));
    console.log('  请运行: cd designer-platform && bash install.sh');
